...
<style>
    ...
    h1 { font-family: -apple-system, sans-serif; color: #0F172A !important; font-weight: 700 !important; letter-spacing: -0.5px; }
    h2, h3, h4 { color: #2563EB !important; font-weight: 600 !important; }
    .stTextInput>div>div>input, .stNumberInput>div>div>input, .stSelectbox>div>div>div { background-color: #FFFFFF !important; color: #0F172A !important; border: 1px solid #CBD5E1 !important; border-radius: 8px !important; }
    ...
    button[data-baseweb="tab"][aria-selected="true"] { color: #2563EB !important; border-bottom-color: #2563EB !important; font-weight: 600; }
...
def load_db_data():
    ...
    if isinstance(res_o, list):
        for row in res_o:
            # 💡 核心设计变量调整：展示和计算时使用真实的业务订单号进行归集
            # 提取可能存在的内部唯一后缀，还原干净的原始单号给用户看
...
# ==========================================
    ...
    for oid, o in orders.items():
        ...
        order_rows.append({"订单编号": o["raw_id"], "订单下发日期": o["date"], "区域省份": o["province"], "客户简称": o["client"], "订购产品明细": o["product"], "接单含税金额": o["amt_with_tax"], "订单项目名称": o["order_p_name"], "💡 累计已确认收入": o["revenue_total"], "⏳ 尚未收货未确收": unrevenue, "累计已回款": o["collect_total"], "待追收尾款": uncollected, "关联源头框架项目": related_p_name})
    ...
    if op_type == "🆕 录入全新数据":
        ...
        if sub_step == "🎯 项目前期录入":
            ...
            if not projects: st.warning("⚠️ 暂无前置项目！")
                ...
                o_id = st.text_input("1. 客户正式订单号 * (同一单号再次录入不同产品会被自动合并保存)")
                ...
                if st.button("⚡ 确认订单下发入库"):
                    if not o_id or not o_province or not o_client or not o_product or not o_p_name: st.error("❌ 请完整填写必填字段！")
                        ...
                        # 💡 核心修复：通过附加时间戳后缀生成唯一联合主键，彻底粉碎一单多品时的数据库拦截问题
                ...
                with st.form("r_form", clear_on_submit=True):
                    ...
                    if st.form_submit_button("信号确收"):
                        if r_amt <= 0: st.error("❌ 金额需大于0元！")
                            ...
                            if res.status_code in [200, 201]: st.success("🎉 签收确收明细流水已记录！"); st.rerun()
            ...
            with st.form("c_form", clear_on_submit=True):
                if not is_legacy_history:
                    if not orders: st.warning("⚠️ 系统内暂无订单。"); st.form_submit_button("不可提交", disabled=True); raw_oid_input = ""
                        ...
                        # 💡 联动修复：多选合并结算时按复合颗粒度进行展现和选择
                        o_opts = {f"订单:{o['raw_id']} | 明细:{o['product']} (尾款:¥{o['amt_with_tax'] - o['collect_total']:.2f})": oid for oid, o in orders.items()}
                        selected_order_labels = st.multiselect("1. 请选择本次合并结算包含的订单/产品明细（支持单选或跨订单自由多选）*", list(o_opts.keys()))
                ...
                if st.form_submit_button("⚡ 执行分摊销账"):
                    cleaned_input = str(raw_oid_input).strip()
                    if not cleaned_input: st.error("❌ 必须选择或填写至少一个识别节点！")
                    elif c_amt <= 0: st.error("❌ 回款金额需大于0元！")
                    else:
                        target_orders = [x.strip() for x in cleaned_input.split(",") if x.strip()]
                        
                        # 💡 核心修复：多期回款同发票号时，插入流水通过随机噪声或行时间戳打散，使其可以在同一张发票下无限记录分批打款流水
                        if len(target_orders) == 1:
                            ...
                            for idx, target_id in enumerate(target_orders):
                                ...
                                each_payload = {"order_ref": target_id, "amount": split_amt, "collection_date": c_date, "invoice_no": f"{c_invoice}_PART{idx}" if c_invoice else f"MUL_{int(datetime.now().timestamp())}_{idx}"}
                                requests.post(f"{SB_URL}/rest/v1/collections", headers=HEADERS, json=each_payload, timeout=5)
                        st.success("🎉 回款多期流水成功打散并精准平摊销账！"); st.rerun()
            ...
            if not orders: st.info("云端暂无订单数据可修改")
                ...
                if st.button("💾 覆写更新订单数据"):
                    if not o_edit_reason.strip(): st.error("❌ 必须填写修改原因！")
                        ...
                        if res.status_code in [200, 204]: st.success("🎉 订单分项明细强审计更新成功！"); st.rerun()
...
# ==========================================
    ...
    f_tabs = st.tabs(["📊 支出分类构成占比", "📜 Journal 复式分录长卷流水", "✍️ 复合借贷账户拆分配置"])
